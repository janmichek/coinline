...
document.addEventListener('mouseup', async (e) => {
  ...
  const { cryptoList, fxRates } = await chrome.storage.local.get(['cryptoList', 'fxRates']);
...
function showPopup(x, y, parsed, cryptoList, fxRates) {
  ...
  popupEl.innerHTML = `
    ...
    <div id="calc-ext-conversion" style="margin-top:6px;display:flex;justify-content:space-between;align-items:center;min-height:20px;font:bold 13px monospace;color:${theme.result}">
      ...
      <button id="calc-ext-copy" style="display:none;background:none;border:1px solid ${theme.border};color:${theme.muted};cursor:pointer;border-radius:4px;font-size:11px;padding:1px 6px;margin-left:6px;flex-shrink:0;outline:none;transition:color .12s ease,border-color .12s ease">Copy</button>
    </div>
  `;
  ...
  copyBtn.onmouseenter = () => {
    copyBtn.style.color = theme.accent;
    copyBtn.style.borderColor = theme.accent;
  };
  copyBtn.onmouseleave = () => {
    copyBtn.style.color = theme.muted;
  ...
  function updateConversion() {
    if (!selectedId) { convText.textContent = ''; copyBtn.style.display = 'none'; lastAmount = null; return; }
    let usdValue;
    if (parsed.type === 'crypto') {
      const source = cryptoList.find(c => c.symbol === parsed.currency);
      if (!source) { convText.textContent = ''; copyBtn.style.display = 'none'; lastAmount = null; return; }
    ...
    if (!coin) { convText.textContent = ''; copyBtn.style.display = 'none'; lastAmount = null; return; }
